...
const pool = new Pool({
  ...
  password: "15346200", // Replace with your PostgreSQL password
...
app.post("/signin", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT email, password, username FROM users WHERE email = $1 AND password = $2",
      [email, password]
    );
...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      res.status(201).json({ msg: "user created successfully", token,userdata:[username, email] });
    } else {
...
const getOrCreateCart = async (userId) => {
  const { rows: cartRows } = await pool.query('SELECT * FROM cart WHERE user_id = $1', [userId]);

  if (cartRows.length === 0) {
    const { rows: newCartRows } = await pool.query('INSERT INTO cart (user_id) VALUES ($1) RETURNING *', [userId]);
...
app.post('/cart', async (req, res) => {
  ...
  try {
    ...
    if (existingItemRows.length > 0) {
      ...
      return res.status(200).json({ msg: 'Cart item updated', cart_item: updatedItemRows[0] });
    }
    ...
    res.status(201).json({ msg: 'Item added to cart', cart_item: newItemRows[0] });
  } catch (error) {
    console.error(error);
...
app.delete('/cart/:cartItemId', async (req, res) => {
  ...
  try {
    const { rows: deletedItemRows } = await pool.query('DELETE FROM cart_items WHERE id = $1 RETURNING *', [cartItemId]);

    if (deletedItemRows.length === 0) {
      return res.status(404).json({ error: 'Cart item not found' });
    }

    res.status(200).json({ msg: 'Item removed from cart', cart_item: deletedItemRows[0] });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
  }
